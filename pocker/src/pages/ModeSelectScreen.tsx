        ...
        <div className="bg-black/50 backdrop-blur-sm border-2 border-white/20 rounded-2xl p-12 w-full max-w-md">
          ...
          <form onSubmit={handleJoinSubmit} className="flex flex-col space-y-4">
            <input
              ...
              disabled={isLoading}
            />
            <GlossyButton type="submit" disabled={!joinCode.trim() || isLoading} className="w-full">
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center justify-center space-y-12 p-4">
      ...
      <div className="flex flex-col space-y-4 w-full max-w-md">
        <GlossyButton onClick={() => onSelectMode("multi")} disabled={isLoading} className="w-full">
          Create Room
        </GlossyButton>

        <GlossyButton onClick={() => setInJoinMode(true)} disabled={isLoading} className="w-full">
          Join Room
        </GlossyButton>

        <GlossyButton onClick={onOpenSettings} disabled={isLoading} className="w-full">
          Settings
        </GlossyButton>
      </div>
    </div>
