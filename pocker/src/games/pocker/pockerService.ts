...
/**
 * Service for interacting with the Pocker game contract
 */
export class PockerService {
  ...
  async finalizeStartGame(
    ...
    console.log('[finalizeStartGame] Starting finalization process...');
    console.log('[finalizeStartGame] Signer address:', signerAddress);
    
    const client = this.createSigningClient(signerAddress, signer);

    // Import the transaction with all auth entries signed
    console.log('[finalizeStartGame] Importing transaction from XDR...');
    ...
    console.log('[finalizeStartGame] Re-simulating transaction with signed auth entries...');
    const simulatedTx = await tx.simulate();
    console.log('[finalizeStartGame] Simulation complete');

    const validUntilLedgerSeq = authTtlMinutes
      ? await calculateValidUntilLedger(RPC_URL, authTtlMinutes)
      : await calculateValidUntilLedger(RPC_URL, DEFAULT_AUTH_TTL_MINUTES);

    console.log('[finalizeStartGame] Valid until ledger:', validUntilLedgerSeq);

    // Sign the transaction envelope and submit
    // IMPORTANT: Use the simulated transaction, not the original tx
    console.log('[finalizeStartGame] Signing and submitting transaction...');
    const sentTx = await signAndSendViaLaunchtube(
      simulatedTx,  // Use simulated transaction
      DEFAULT_METHOD_OPTIONS.timeoutInSeconds,
      validUntilLedgerSeq
    );
    
    console.log('[finalizeStartGame] Transaction sent!');
    console.log('[finalizeStartGame] Transaction hash:', sentTx.getTransactionResponse?.hash);
    console.log('[finalizeStartGame] Transaction status:', sentTx.getTransactionResponse?.status);
    console.log('[finalizeStartGame] Result:', sentTx.result);
    
    // Return the full sent transaction object so caller can check status
    return sentTx;
  }
...
// Example: const pockerService = new PockerService(contractId);
