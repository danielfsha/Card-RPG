...
// Unit tests for the card-rpg contract using a simple mock GameHub.
...
use crate::{Error, CardRpgContract, CardRpgContractClient};
...
fn setup_test() -> (
    ...
    // Deploy card-rpg with admin and GameHub address
    let contract_id = env.register(CardRpgContract, (&admin, &hub_addr));
    let client = CardRpgContractClient::new(&env, &contract_id);

    // Register card-rpg as a whitelisted game (mock does nothing)
