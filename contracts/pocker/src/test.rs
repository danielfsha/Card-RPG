...
// Unit tests for the pocker contract using a simple mock GameHub.
...
use crate::{Error, PockerContract, PockerContractClient};
...
fn setup_test() -> (
    ...
    // Deploy pocker with admin and GameHub address
    let contract_id = env.register(PockerContract, (&admin, &hub_addr));
    let client = PockerContractClient::new(&env, &contract_id);

    // Register pocker as a whitelisted game (mock does nothing)
