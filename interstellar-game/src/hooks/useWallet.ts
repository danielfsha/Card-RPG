  ...
  const disconnect = useCallback(async () => {
    try {
      if (walletType === "dev") {
        ...
        // Disconnect from StellarWalletsKit
        try {
          await StellarWalletsKit.disconnect();
        } catch (e) {
          console.log("StellarWalletsKit disconnect error (non-critical):", e);
        }
      }
      storeDisconnect();
    } catch (error) {
      console.error("Disconnect error:", error);
      // Still disconnect from store even if wallet disconnect fails
      storeDisconnect();
    }
