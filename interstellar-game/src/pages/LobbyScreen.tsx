...
export function LobbyScreen({ onStartGame }: LobbyScreenProps) {
  ...
  const handleGuestImportAndFinalize = async () => {
    ...
    try {
      ...
      const fullTxXDR = await interstellarService.importAndSignAuthEntry(
        p1AuthEntryXDR,
        publicKey!,
        points,
        signer,
      ...
      // Finalize and submit
      const sentTx = await interstellarService.finalizeStartGame(
        fullTxXDR,
        publicKey!,
        signer,
      );
      ...
      const { InterstellarService: VerifyService } =
        await import("../games/interstellar/interstellarService");
      const { INTERSTELLAR_CONTRACT: VerifyContract } =
  ...
  const handleHostStartGame = async () => {
    ...
    try {
      ...
      const authEntryXDR = await interstellarService.prepareStartGame(
        newSessionId,
        player1,
      ...
      const pollInterval = setInterval(async () => {
        try {
          const game = await interstellarService.getGame(newSessionId);
          if (game) {
