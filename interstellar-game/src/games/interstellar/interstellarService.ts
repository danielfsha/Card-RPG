import { Client as InterstellarClient, type Game } from "./bindings";
import {
  NETWORK_PASSPHRASE,
  RPC_URL,
...
/**
 * Service for interacting with the Interstellar ZK Fighter game contract
 */
export class InterstellarService {
  private baseClient: InterstellarClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new InterstellarClient({
  ...
  private createSigningClient(
    ...
    return new InterstellarClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new InterstellarClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
      publicKey: player2,
