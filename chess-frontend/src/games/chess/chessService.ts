import { Client as ChessClient, type Game } from './bindings';
...
/**
 * Service for interacting with the Chess game contract
 */
export class ChessService {
  private baseClient: ChessClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new ChessClient({
  ...
  private createSigningClient(
    ...
    return new ChessClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new ChessClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
...
// Example: const chessService = new ChessService(contractId);
